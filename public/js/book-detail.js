...
import { fetchBookById, fetchGutendexBookById } from "./api.mjs";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Determine if the book is Gutendex (numeric ID)
  if (/^\d+$/.test(bookId)) {
  ...
  const image = isGutendex
    ? book.image
    : book.volumeInfo.imageLinks?.thumbnail || "https://via.placeholder.com/200x300";
  ...
  document.getElementById("add-favorite").addEventListener("click", () => {
    ...
    if (!favorites.some((f) => f.id == bookId)) {
      favorites.push({ id: bookId, title, author: authors, image });
  ...
  document.getElementById("add-cart").addEventListener("click", () => {
    ...
    if (existing) {
      ...
      cart.push({ id: bookId, title, author: authors, image, quantity: 1 });
    }
